    ...
    try:
        ...
        if certificate.course.certificate_show_qr:
            try:
                ...
                # Generate QR code image with same style as website
                qr_img = qr.make_image(fill_color="#000000", back_color="white")
                qr_img = qr_img.resize((100, 100), Image.Resampling.LANCZOS)

                # Create white background with border for QR code (same as website)
                ...
                # Add QR label with better positioning
                ...
                # Fallback to improved placeholder if QR generation fails
                qr_placeholder_x = width - 160
                qr_placeholder_y = height - 150

                # Create styled placeholder box
                ...
                for i in range(3, 97, 8):
                    for j in range(3, 97, 8):
                        if (i + j) % 16 == 0:
                            placeholder_draw.rectangle([i, j, i+4, j+4], fill=secondary_color)

                # Add text overlay
                placeholder_draw.text((55, 35), "QR", fill=secondary_color, font=name_font, anchor="mm")
                placeholder_draw.text((55, 75), certificate.certificate_code, fill=secondary_color, font=small_font, anchor="mm")

                # Paste placeholder
                img.paste(placeholder_bg, (qr_placeholder_x-5, qr_placeholder_y-5))

                # Add label
                draw.text((qr_placeholder_x + 50, qr_placeholder_y + 115), "Сканируйте для проверки",
                         fill=secondary_color, font=small_font, anchor="mm")
